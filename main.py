...
# Imports needed for search4_source()
import sys
import io
import requests
...
def source(bot, update):
    ...
    if authorized_room is True and username is not None:
        # Get photo's file_id
        photo_id = update.message.reply_to_message.photo[2].file_id
        # Get the download link from Telegram
        file = bot.get_file(file_id=photo_id)
        # Download the photo
        file.download(custom_path="photo.jpg",timeout=10)
                        ...
                        if processResults:
                            ...
                            if int(results['header']['results_returned']) > 0:
                                ...
                                if float(results['results'][0]['header']['similarity']) > float(
                                    ...
                                    pic_similarity = str(results['results'][0]['header']['similarity'])
                                ...
                                print('no results... ;_;')
                                bot.send_message(chat_id=update.message.chat_id,text="No results")
